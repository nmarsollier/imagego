...
func (ec *executionContext) resolveEntity(
	...
	case "Image":
		...
		if err != nil {
			return nil, fmt.Errorf(`finding resolver for Entity "Image": %w`, err)
		}
		switch resolverName {
		...
		case "findImageByID":
			...
			if err != nil {
				return nil, fmt.Errorf(`unmarshalling param 0 for findImageByID(): %w`, err)
			}
			entity, err := ec.resolvers.Entity().FindImageByID(ctx, id0)
			if err != nil {
				return nil, fmt.Errorf(`resolving Entity "Image": %w`, err)
			}
...
func entityResolverNameForImage(ctx context.Context, rep EntityRepresentation) (string, error) {
	for {
		var (
